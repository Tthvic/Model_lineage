...
from src.small_model.legacy_network import TransformerEncoder, VectorRelationNet
...
def load_legacy_checkpoint(model_path, encoder, relation_net, device):
    ...
    try:
        ...
        if 'encoder_state_dict' in checkpoint:
            ...
            # This part is tricky without seeing the file. 
            # Based on typical usage, it might be:
            # checkpoint = {'encoder': ..., 'relation': ...}
            ...
            if 'relation' in checkpoint:
                ...
                # Attempt to load if keys match directly (unlikely for two models)
                
    except Exception as e:
        print(f"Error loading checkpoint: {e}")
        print("Tip: If you see an UnpicklingError or 'invalid load key', the file might be a Git LFS pointer or corrupt.")
        print("     Please verify the file size (should be > 1MB) and replace it with the actual .pth file.")
        sys.exit(1)

def main():
    parser = argparse.ArgumentParser(description="Evaluate Legacy Demo Model")
    ...
    # Load Checkpoint
    # if check_lfs_file(args.checkpoint):
    #     print(f"Error: The file {args.checkpoint} is a Git LFS pointer, not the actual model weights.")
    #     print("Please run 'git lfs pull' to download the actual files.")
    #     sys.exit(1)
    ...
    try:
        ...
        if isinstance(checkpoint, dict):
            ...
            # Heuristic to load state dict
            if 'encoder_state_dict' in checkpoint:
                encoder.load_state_dict(checkpoint['encoder_state_dict'])
                relation_net.load_state_dict(checkpoint['relation_net_state_dict'])
                print("Loaded encoder and relation_net from explicit keys.")
            elif 'model_state_dict' in checkpoint:
                 # Try to load into encoder?
                 pass
            else:
                # Try to match keys
                pass
    ...
    with torch.no_grad():
        for fname in files:
            ...
            try:
                ...
                # We need a negative pair too. 
                # dataloader824.py uses 'fb_afea' and 'fcfea' for negative?
                # Or maybe 'ind_fea'?
