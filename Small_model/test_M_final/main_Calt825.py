...
from dataloader824 import create_dataloader
...
def testparent_childandgrandparent_child(encnet, prenet, save_dir, epoch):
    ...
    results = {"parent_child": [], "grandparent_child": []}  # store parent_child and grandparent-grandchild data
    encnet.eval()
    prenet.eval()
    ...
    # Reload data into model and test
    test_results = {"parent_child": [], "grandparent_child": [],"non": []}
    for category, data_list in reload_results.items():
        for data in data_list:
            # Reload features
            cfea= data["cfea"].to('cuda:0') # ensure correct device
    ...
    for i in range(10):
        ...
        print(test_results["non"][i]['similarity'],"test_results_nonlineage")
    # print(f"[Epoch {epoch}] Test results saved: {test_save_path}")
...
def test_fpr(encnet, prenet, test_loader, contrastive_loss_fn, epoch, name):
    ...
    # Initialize per-category storage for similarities and counts
    results = {
...
def main():
    ...
    def test():
        epoch=1
        testparent_childandgrandparent_child(encnet, prenet,save_dir="./framework",epoch=epoch)
    test()
