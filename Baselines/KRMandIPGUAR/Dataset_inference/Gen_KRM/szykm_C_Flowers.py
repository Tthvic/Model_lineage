...
import random
import argparse
from szykm_C_Calt import get_p_names
...
class Parser:
    ...
    def set_arguments(self):
        ...
        ###############################################
        self.parser.add_argument('--num_class', type=int, default=dimdict['Flowers_c'])
...
if __name__ == "__main__":
    ...
    set_seed(111)
    models = []
    childmodels=os.listdir('./../../../../szy2025/train/train_res18/NONCmodels')

    model_indexdict={}
    for childname in childmodels:
        if not 'Flowers' in childname:
            continue
        pname = get_p_names(childname)
        parentindex=extract_number_from_filename(pname)
        model_indexdict[childname] = oriclass[parentindex]
        print(pname,childname,"childname")

    # 2. load the student model
    ######生成子模型的对抗样本
    childmodels = list(model_indexdict.keys())
    for parentname in childmodels:
        ...
        if os.path.exists(os.path.join('./Flowers_adv', parentname)):
            continue
        ####SZY注释，这里的parentmodel不是parentmodel，而是可疑的子模型
        parentmodel = load_cmodels(parentname)
        ...
        torch.save(dict0, os.path.join('./NON_C_Flowers_adv', parentname))