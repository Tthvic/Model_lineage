...
def get_p_names(file_name):
    # 使用正则表达式匹配父模型名称部分
    import re
    match = re.search(r"Flowers_\d+_(.+?\.pth)\.pth$", file_name)
    if match:
...
def ipguard_function(args,parentname,childnames):
    ...
    feature_dir=f"./../Dataset_inference/Gen_KRM/NON_Flowers_adv/"+parentname
...
if __name__ == "__main__":
    ...
    avgtotalaccs=torch.mean(torch.stack(totalaccs))
    print(avgtotalaccs,"avgtotalaccs")