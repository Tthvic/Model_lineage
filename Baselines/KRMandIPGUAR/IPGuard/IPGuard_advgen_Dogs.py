...
def embeddings_extractor(args,modelname):
    ...
    select_class=pclass[extract_number_from_filename(modelname)]
    ...
    torch.save(train_d,'./Dogs/'+modelname+'train_features.pt')
    torch.save(train_adv, './Dogs/'+modelname+'train_advs.pt' )
    test_d, test_adv = get_black_vulnerabilty(args, test_loader, student)
    torch.save(test_d,'./Dogs/'+modelname+'test_features.pt')
    torch.save(test_adv, './Dogs/'+modelname+'test_advs.pt' )
...
if __name__ == "__main__":
    ...
    for modelname in modelnames:
        ...
        feature_dir=os.path.join("./feas_Dogs/", modelname)
        args.feature_dir = feature_dir
        if not os.path.exists(feature_dir):
            os.makedirs(feature_dir)
        print("Feature Directory:", feature_dir)
        args.num_class=dimdict['Dogs_p']
