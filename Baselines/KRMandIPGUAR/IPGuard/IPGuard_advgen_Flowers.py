...
# from dogs import *
# from IPGuard_advgen_Calt import load_pmodels
...
def embeddings_extractor(args,modelname):
    ...
    torch.save(train_d, f"{args.feature_dir}/train_{modelname}_features.pt")
    torch.save(train_adv, f"{args.feature_dir}/train_{modelname}_advs.pt")
    # torch.save(train_d,'./Dogs/train_features.pt')
    # torch.save(train_adv, './Dogs/train_advs.pt' )
    test_d, test_adv = get_black_vulnerabilty(args, test_loader, student)
    # torch.save(test_d,'./Dogs/test_features.pt')
    # torch.save(test_adv, './Dogs/test_advs.pt' )
    torch.save(test_d, f"{args.feature_dir}/test_{modelname}_features.pt")
    torch.save(test_adv, f"{args.feature_dir}/test_{modelname}_advs.pt")
...
if __name__ == "__main__":
    ...
    for modelname in modelnames:
        ...
        feature_dir=os.path.join("./feas_Flowers/", modelname)
        args.feature_dir = feature_dir
        if not os.path.exists(feature_dir):
            os.makedirs(feature_dir)
        print("Feature Directory:", feature_dir)
        args.num_class=dimdict['Flowers_p']
