...
from Small_model.test_R_final.network3 import *
from Small_model.test_R_final.dataloader810 import create_dataloader,EmbeddingDataset,pathdict
...
def test(encnet, prenet, test_loader, contrastive_loss_fn, epoch, name):
    ...
    with torch.no_grad():  # Disable gradient computation during testing
        for i, (posname, negname, data, falsedata) in enumerate(test_loader):
            ...
            # Iterate over samples in the batch and categorize by filename
            for j in range(batch_size):
                # Assume pos/neg have the same filename, so use posname[j]
                fname = posname[j]
                if "Calt" in fname:
                    category = "Calt"
                ...
                # Update counts by similarity threshold
    ...
    # If needed, accumulate contrastive loss here (not computed in this example)
...
def framework( encnet, prenet, save_dir, epoch):
    ...
    # results = {"parent_child": [], "grandparent_child": [],"non_lineage":[]}  # Store complete data for parent_child and grandparent-grandchild
    encnet.eval()
    prenet.eval()
...
def main():
    ...
    def test():
        epoch=3
        save_dir='Small_model/test_R_final/framework'
